"""RFC-023 optimization check: does batched encoding help image indexing throughput?

Follow-up to siglip_bakeoff.py and siglip_quantization_check.py, testing
the one optimization flagged from the start as relevant to bulk indexing
specifically rather than live search: batching. A search query is always
one text string -- batching cannot help that path. Indexing 100k images
is the opposite case: every prior script here encoded images one at a
time, which wastes whatever parallelism the CPU has per forward pass.

Uses the bake-off's winning checkpoint (google/siglip2-base-patch16-384),
fp32, CPU -- same methodology as the other checks in this directory, so
numbers stay comparable.

Two things measured, not just one:
  1. Speed -- amortized seconds/image at each batch size, against a
     freshly measured batch-of-1 baseline (matching how every prior
     script in this directory encoded images, so this is the correct
     comparison point, not last session's numbers from a different
...
Run:
    python siglip_batching_check.py
"""
...
IMAGE_BATCH_SIZES = [1, 2, 4, 8, 16, 32]
...
def encode_text_batches(
    ...
    for batch_texts in chunk(texts, batch_size):
        inputs = processor(text=batch_texts, padding="max_length", return_tensors="pt")
        with torch.no_grad():
...
def main() -> None:
    ...
    results_path = Path(__file__).with_name("batching_check_results.json")
